...
Timeline rows live in `src/data/history.ts` and render in a table with columns **Period**, **City**, **State**, and **Notes**. Stops with `lat`/`lng` also render maps below the table via `MapSnapshot.astro` (satellite by default). Set `mapVariant: 'streetview'` for Street View instead of aerial — e.g. Williamsburg `1989—1992` and Hattiesburg `1992` (addresses stay hidden in the caption). Optional `streetViewHeading` aims the camera. Date ranges use em dashes (e.g. `1996—1999`).
